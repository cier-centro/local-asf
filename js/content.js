...
function isInString(phrase, word) {
...
function getVideosArray(bimester) {
  ...
  $.each(selectedArray["VIDEOS"][bimesterKey], function(index, object) {
    if ((isString(object)) && (isInString(object, ".mp4")))
...
function createGuidesButtonsBySort(bimester, sortArray, sort) {
  ...
  for (var i = 0; i < sortArray.length; i++) {
    var guideText = (isInString(sort, "DOCENTE")) ? "Guía Docente" : (isInString(sort, "ESTUDIANTE")) ? "Guía Estudiante" : "Guía";
    guideText += (i > 0) ? " " + (i + 1) : "";
...
function getGuideArrayBySort(bimester, sort) {
  ...
  $.each(selectedArray, function(index, object) {
    ...
    if ((isString(object)) && ((isInString(object, bimesterKey)) || (isInString(object, "COMPLETO"))) && (isInString(object, sort)) && (isInString(object, ".pdf")))
